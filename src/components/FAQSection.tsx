    ...
    <section className="py-16 md:py-20 px-4 md:px-6">
      <div className="container mx-auto max-w-3xl">
        ...
        <Accordion type="single" collapsible className="space-y-3">
          {faqs.map((faq, index) => (
            <AccordionItem 
              key={index} 
              value={`item-${index}`}
              className="card-glow rounded-xl px-6 border-none"
            >
              <AccordionTrigger className="text-left font-semibold text-sm md:text-base hover:no-underline py-5">
                {faq.question}
              </AccordionTrigger>
              <AccordionContent className="text-foreground/70 text-sm md:text-base pb-5">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
        ...
        <div className="text-center mt-12">
          <p className="text-foreground/60 mb-4">Ainda tem dúvidas?</p>
          <Button 
            size="lg" 
          ...
          <p className="text-xs text-foreground/50 mt-3">Garantia de 30 dias ou seu dinheiro de volta</p>
        </div>
      </div>
    </section>
  );
